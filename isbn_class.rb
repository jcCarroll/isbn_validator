def valid_isbn?(isbn)
    ...
    if valid_isbn_ten_length?(isbn) && valid_isbn_ten_check_sum?(isbn) # checks if length of isbn is 10 digits and returns true or false
        ...
        valid_isbn_thirteen_length?(isbn) && valid_isbn_thirteen_check_sum?(isbn)
        true
    else
        false
    end
        ...
        false
                    ...
                    def valid_isbn_thirteen_check_sum?(isbn)
                        ...
                        check_sum = (10 - pre_check_sum) # sets check_sum equal to ten minus pre_check_sum
                            if check_sum == thirteen_digit_array[12] # looks to see if check_sum is equal to position 12 of thirteen_digit_array and returns true or false
                                true
                            else
                                false
                            end
